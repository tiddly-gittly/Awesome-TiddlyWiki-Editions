...
/**
 * 已下载的文件记录在这里面，防止重复下载
 * key 为 tiddler['ate.uri']
 * value 为 `${distEditionDir}/${distEditionName}` 是文件路径
 */
const downloadFileMap = {
  // DEBUG: console
  'https://keatonlao.github.io/tiddlywiki-xp/': 'dist/editions/TiddlyWiki%20XP.html',
  'https://tiddly-gittly.github.io/Tiddlywiki-NodeJS-Github-Template/index-full.html': 'dist/editions/ItonNote.html',
};
    ...
    try {
      ...
      if (
        ...
        if (downloadFileMap[tiddler['ate.uri']]) {
          try {
            // prevent error cp: xxx/Awesome-TiddlyWiki-Editions/dist/editions/ItonNote.html and dist/editions/ItonNote.html are identical (not copied). which is ok because we just download our file there
            await $`cp ${downloadFileMap[tiddler['ate.uri']]} ${distEditionDir}/${distEditionName}`;
          } catch (error) {
            if (!error.message.includes('are identical (not copied)')) {
              throw error;
            }
          }
        } else {
        ...
        editionInfos.push(newEditionInfo);
      }
    } catch (e) {
...
await fs.writeJSON(path.join(distDir, 'editions.json'), editionInfos);
